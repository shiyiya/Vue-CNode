<template>
  <section class="list">
    <router-link :to="{name:'user',params:{id:data.author.loginname}}">
      <img :src="data.author.avatar_url" :alt="data.author.loginname">
    </router-link>
    <div class="content">
      <router-link :to="{name:'topic',params:{id:data.id}}">
        <h2>{{data.title}}</h2>
      </router-link>
      <span>{{data.author.loginname}}</span>
      <span v-if="data.create_at">发表时间：{{`${new Date(data.create_at).getFullYear()}-${new Date(data.create_at).getMonth()+1}`}}</span>
      <span>最后回复：{{data.last_reply_at}}</span>
      <div class="tag">
        <span v-if="data.good">good</span>
        <span v-if="data.top">top</span>
      </div>
      <div class="view">
        <span v-if="data.reply_count">{{data.reply_count}} </span>
        <span v-if="data.visit_count">/{{data.visit_count}}</span>
      </div>
    </div>
  </section>
</template>
<script>
export default {
  name: "list",
  props: {
    data: Object
  }
};
</script>
<style scoped>
.list {
  position: relative;
  border-bottom: 2px solid #42b98361;
  padding: 0.2em 0;
}
.list img {
  display: inline-block;
  width: 40px;
  height: 40px;
  border-radius: 50%;
  margin: 0 1em;
}

.list .content {
  display: inline-block;
  /* position: absolute; ///////////////////*/
}
.view,
.tag {
  position: absolute;
  right: 0;
  display: inline-block;
}

.view > span:nth-child(1) {
  color: #42b983;
}

.content > span {
  margin: 0 1em 0 0;
  color: #8796a3;
  font-size: small;
}
.tag {
  top: 2px;
  background-color: #42b983;
  border-radius: 50%;
  padding: 0 3px;
  margin: 0 !important;
  color: white !important;
}
</style>

